...
async def analyze_change_impact(
    ...
    # ── Compute dependents that @api.depends on this field ────────────────────
    if field_name:
        compute_dep_rows = await async_query(
            db_path,
            """
            SELECT dd.model_name, dd.method_name, dd.depends_fields
        ...
        for row in compute_dep_rows:
            ...
            # Confirm it truly contains this field (not just substring match)
            matching = [d for d in deps if field_name in d]
            if matching:
    ...
    # ── View references ───────────────────────────────────────────────────────
    if field_name:
        view_field_rows = await async_query(
            db_path,
            """
            SELECT ver.view_xml_id, v.name, v.view_type
        ...
        view_refs = [
            f"{r['view_xml_id']} uses field {field_name}"
            for r in view_field_rows
        ]
        # Also check views that list this field in field_names JSON
        view_json_rows = await async_query(
            db_path,
            "SELECT xml_id, name FROM views WHERE model = ? AND field_names LIKE ?",
            (model_name, f"%{field_name}%"),
        )
        for row in view_json_rows:
            ref = f"{row['xml_id']} references field {field_name}"
            if ref not in view_refs:
                view_refs.append(ref)
        impact["view_refs"] = view_refs
        impact_score += len(view_field_rows)

    elif method_name:
        # Method name may appear as button action
        view_btn_rows = await async_query(
            db_path,
            """
    ...
    if field_name:
        btn_vis_rows = await async_query(
            db_path,
            """
    ...
    # ── Constraints ───────────────────────────────────────────────────────────
    if field_name:
        constrain_rows = await async_query(
            db_path,
            """
        ...
        for row in constrain_rows:
            ...
            if field_name in fields:
                constrained_by.append(f"{row['method_name']} constrains {field_name}")
        impact["constrained_by"] = constrained_by
        impact_score += len(constrained_by)

    # ── State machine ─────────────────────────────────────────────────────────
    if field_name:
        sm_row = await async_query(
            db_path,
            "SELECT * FROM state_machines WHERE model_name = ? AND field_name = ?",
            (model_name, field_name),
        )
        if sm_row:
            transitions = json_col(sm_row[0], "transitions", [])
            impact["state_machine"] = (
                f"{field_name} field drives workflow with "
    ...
    if field_name:
        rr_rows = await async_query(
            db_path,
            "SELECT name, domain_force FROM record_rules WHERE model_name = ? AND domain_force LIKE ?",
            (model_name, f"%{field_name}%"),
        )
        impact["record_rules"] = [
